...
function Users({ users, mongoUser }: Props) {
  ...
  return (
    <>
      <div className="group relative w-[30%] hidden lg:block bg-[#1a182e] px-3 py-4">
        <button
        ...
        <div className={` h-full text-white flex-col w-full  `}>
          <div className="card h-1/2 px-2 py-3">
            ...
            <div className="flex flex-col mt-4 gap-1 rounded-lg">
              {users.length
                ? users?.map((user, idx) => (
                    <div className="rounded-lg px-2 text-xs" key={idx}>
                      <UserCard
                        name={user["name"]}
                        image={user["image"]}
                        userId={user["_id"]}
